...
def load_data():
    if not os.path.exists(DATA_PATH):
        st.error(f"❌ Dataset not found at {DATA_PATH}. Please ensure the CSV is in the repository.")
        st.stop()
...
# --- NAVIGATION ---
st.sidebar.title("Deriv Risk Hub")
...
if page == "📊 Dashboard":
    ...
    with c1:
        reg_list = sorted(df_all['region'].dropna().unique().tolist())
        reg_filter = st.multiselect("Region", options=reg_list, default=reg_list)
    with c2:
        pri_list = sorted(df_all['priority'].dropna().unique().tolist())
        pri_filter = st.multiselect("Tier", options=pri_list, default=pri_list)
    with c3:
        stat_list = sorted(df_all['Status'].unique().tolist())
        # FIXED: Changed from stat_filter to status_filter to match the filter logic below
        status_filter = st.multiselect("Status", options=stat_list, default=stat_list)

    filtered_df = df_all[
        (df_all['region'].isin(reg_filter)) & 
        (df_all['priority'].isin(pri_filter)) &
        (df_all['Status'].isin(status_filter)) # This now matches the multiselect variable
    ]
