...
def load_models():
    try:
        churn_model = joblib.load("churn_risk_model.pkl")
        encoders = joblib.load("feature_encoders.pkl")
        lookup_dict = joblib.load("partner_lookup_model.pkl")
        return churn_model, encoders, lookup_dict
    except Exception as e:
        st.error("❌ Failed to load model files. Ensure all .pkl files are in repo root.")
        st.stop()
...
def load_data():
    try:
        df = pd.read_csv("large_partner_churn_dataset_24000.csv")
        ...
        st.error("❌ Dataset file not found or invalid.")
        st.stop()
...
def predict_churn_risk(partner_data, model, encoders):
    ...
    X = partner_data[features].astype(float).values.reshape(1, -1)
